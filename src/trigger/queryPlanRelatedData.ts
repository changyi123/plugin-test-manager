...
const runScript = async () => {
  ...
  try {
    ...
    if (versionKeys && versionKeys.length > 0) {
      ...
      // 然后查询版本下的需求事项
      const versionReqIqls = versionKeys.map(key => `item in hierarchicalQuery('${key}', 0, 1)`).join(' or ');
      ...
      const versionPlanIqls = versionKeys.map(key => `item in hierarchicalQuery('${key}', 0, 2)`).join(' or ');
      ...
      // 然后查询需求下的测试计划
      const reqPlanIqls = requirementKeys.map(key => `item in hierarchicalQuery('${key}', 0, 1)`).join(' or ');
      ...
      const directPlanIql = `'test_manager_type' = 'TestPlan' and key in [${planKeys.map(key => `'${key}'`).join(',')}]`;
    ...
    // 0. 查询测试计划详情
    console.log('\n--- 第0步：查询测试计划详情 ---');
    const planIql = `'test_manager_type' = 'TestPlan' and id in [${finalPlanIds.map(id => `'${id}'`).join(',')}]`;
    ...
    if (planResult?.payload?.items) {
      ...
      console.log('测试计划详情:', result.plans.map(plan => ({ id: plan.id, key: plan.key, name: plan.name })));
    } else {
    ...
    if (includeExecutions) {
      console.log('\n--- 第1步：查询测试执行任务 ---');
      const executionIql = `'test_manager_linkItems' in [${finalPlanIds.map(id => `'${id}'`).join(',')}] and 'test_manager_linkType' = 'ExecutionLinkPlan' and 'test_manager_type' = 'TestExecution'`;
      ...
      if (executionResult?.payload?.items) {
        ...
        console.log('测试执行任务详情:', result.executions.map(exec => ({ 
          id: exec.id, 
          key: exec.key, 
          name: exec.name,
    ...
    if (includeRuns && executionIds.length > 0) {
      console.log('\n--- 第2步：查询测试执行 ---');
      const runIql = `'test_manager_type' = 'TestRun' and 'test_manager_linkItems' in [${executionIds.map(id => `'${id}'`).join(',')}] and 'test_manager_linkType' = 'RunLinkExecution'`;
      ...
      if (runResult?.payload?.items) {
        ...
        console.log('测试执行详情:', result.runs.map(run => ({ 
          id: run.id, 
          key: run.key, 
          name: run.name,
    ...
    if (includeDefects && defectIds.length > 0) {
      ...
      if (defectResult?.payload?.items) {
        ...
        console.log('缺陷详情:', result.defects.map(defect => ({ 
          id: defect.id, 
          key: defect.key, 
          name: defect.name,
