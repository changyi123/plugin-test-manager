...
// 防止重复 itemSaveSuccess cb 被重复监听
let lockItemId: string | null = null;
/** 监听事项创建成功 */
export const useOnItemCreateSuccess = cb => {
  const memoizedCallback = React.useCallback(
    params => {
      // 监听 key 为 TEST_MANAGER_PLUGIN_KEY 的事件
      if (lockItemId !== params.itemId && params?.extraData?.key === TEST_MANAGER_PLUGIN_KEY) {
        lockItemId = params.itemId;
        cb(params);
      }
    },
    [cb],
