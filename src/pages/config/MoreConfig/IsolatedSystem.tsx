...
const Descriptions = [
  '测试管理插件会在初始化阶段内置测试用例，测试计划，测试执行任务三种事项类型，请勿删除。若内置三种事项类型被删除，测试管理系统隔离配置将不能被开启',
  '测试管理系统隔离配置开启后，事项类型关联配置将不能被修改。测试管理事项关联配置会默认关联测试管理插件内置的三种事项类型，全部测试空间事项类型关联配置会被更新为内置的三种事项类型',
  '测试管理系统隔离配置开启后，测试管理关联的事项类型创建的事项会被隐藏，事项将不会出现在空间视图中，也无法通过 IQL 中搜索查询',
  '事项管理的视图面板内所有事项类型下拉选择框无法选择测试管理插件内置的三种事项类型',
];
  ...
  const handleSwitchChange = async checked => {
    if (checked) {
      ...
      if (builtinItemTypes.length < 3) {
        throw message.error('测试管理内置的事项类型不存在，无法开启此配置');
      }

      await confirmPromisify({
        title: '警告',
        width: 600,
        content: '当前操作会使全部测试空间事项类型关联配置类型被覆盖！！！',
