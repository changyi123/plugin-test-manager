...
const PlanPageLayout: React.FC<any> = ({ setApprovalEntry }) => {
  const {
    workspaceKey,
  ...
  const [activeType, setActiveType] = useState<'TestPlan'>('TestPlan');
  ...
  // 获取测试计划关联的全部测试用例 id
  const { data: planLinkCaseIds, refreshAsync: planLinkCaseIdRefresh } = useGetApprovalLinkCaseIds({
    workspaceKey,
    type: 'TestPlan',
    testApprovalId: activeType === 'TestPlan' ? selectedTestApproval?.objectId : null,
  });
  ...
  useUpdateEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeType, selectedExecution?.objectId, selectedTestApproval]);
  ...
  const createExecution = useCallback(
    async (caseIds = [], createNext = false, caseVersion = {}) => {
      const config = await getExecutionDefaultConfig(selectedTestApproval);
    ...
    [createItemUseModal, selectedTestApproval, t],
  );
  ...
  const refreshTreeAndScopeTestCase = useCallback(async () => {
    ...
    await planLinkCaseIdRefresh();
    pageLeftRef.current?.refresh?.();
  }, [activeType, planLinkCaseIdRefresh]);
  ...
  const addTestExecutionToPlan = React.useCallback(
    ...
    [selectedTestApproval?.objectId, t],
  );
