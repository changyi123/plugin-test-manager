...
/** 校验脑图数据是否合法 */
export const validateMinderData = ({ rootNode, rootNodeLevel = 0 }, t) => {
  // const rootNode = cloneDeep(_rootNode);
  const scopedT = (key, ...args) => t(`page.xMindImport.draftEditorStep.${key}`, ...args);
  const globalT = (key, ...args) => t(key, ...args);
  ...
  const buildValidatorStrategies = (node, parent) => {
    const validatorStrategies = {
      ...
      moduleNodeLevelMaxCount: node => {
        ...
        let level = rootNodeLevel;
        let nodeParent = node;
        while (nodeParent?.data?.type === MinderNodeType.Module) {
          level++;
      ...
      moduleNodeNameRepeat: (node, parent) => {
        ...
        if (moduleNameList.indexOf(node.data.text) !== moduleNameList.lastIndexOf(node.data.text)) {
