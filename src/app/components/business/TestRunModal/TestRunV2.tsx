...
const TestRunV2: React.FC<TestRunType> = props => {
  ...
  const [canExecNext, setCanExecNext] = useState(true);
  ...
  // 执行下一个测试用例
  const nextTestRun = React.useCallback(async () => {
    event.emit();
    const { id, hasNext } = await getNext();
    if (hasNext !== canExecNext) setCanExecNext(hasNext);

    console.info('canExecuteTestRunIdSequence', id, hasNext);
    if (!hasNext) {
      message.info('本页最后一条可执行的用例了');
    }
    setTestRunId(id);
  }, [canExecNext, event, getNext]);
...
export default React.memo(TestRunV2);
