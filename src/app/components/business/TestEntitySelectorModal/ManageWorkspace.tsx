...
const ManageWorkspaceModal: React.FC<TestEntitySelectorProps> = props => {
  const { t } = useI18n();
  const {
    planId,
    actionRef,
  ...
  const [currentWorkspaceKey, setCurrentWorkspaceKey] = useState();
  // 是否是测试缺陷类型
  ...
  const testDetailSelectorNode = React.useMemo(() => {
    ...
    return (
      <div className={cx('import-by-workspace-wrap')}>
        {!isSingleMode && (
...
export default React.memo(ManageWorkspaceModal);
